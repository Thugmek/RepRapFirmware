...
void HX711MULTI::set_gain(size_t gain) {
...
bool HX711MULTI::tare(size_t times, uint16_t tolerance) {
		...
		if (NULL!=result) {
			for (j = 0; j < COUNT; ++j) {
				if (digitalRead(DOUT[j])) {
					SetBit(result[j], 23-i);
				} else {
					ClearBit(result[j], 23-i);
				}
			}
		}
		digitalWrite(PD_SCK, LOW);
